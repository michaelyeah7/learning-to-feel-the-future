...
class ACTPolicyWithHSA(ACTPolicy):
    ...
    def __call__(self, qpos, image, actions=None, is_pad=None, vq_sample=None):
        ...
        # Handle list format for image data (when tactile is separate)
        # Extract RGB for base ACT policy
        if isinstance(image, list):
            # List format: [rgb_tensor, tactile_tensor]
            # Base ACT policy only needs RGB
            rgb_image = image[0]
        else:
            # Tensor format: assume all cameras in one tensor
            rgb_image = image
        
        # Call base ACT policy with RGB only (this will normalize images)
        base_output = super().__call__(qpos, rgb_image, actions, is_pad, vq_sample)
