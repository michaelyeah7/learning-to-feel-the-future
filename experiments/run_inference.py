...
def main(args):
   ...
   # robot init
    ...
    model = Imitate_Model(ckpt_dir='./ckpt/act_peg', ckpt_name=model_name)

    model.loadModel()
    print("model init success...")

    # Initialize the parameters
    episode_len = 90000  # The total number of steps to complete the task. Note that it must be less than or equal to parameter 'episode_len' of the corresponding task in file 'ModelTrain.constants'
    t=0
    last_time = 0
    ...
    while t < episode_len:
        ...
        # left arm (jaw tip position) limit:  300>x>-410  -700<Y<-210  z>42;
        # right arm (jaw tip position) limit:  410>x>-250  -700<Y<-210  z>42;
        t1 = time.time()
